...
public struct Rational : IComparable
{
    ...
    public static Rational operator *(Rational a, Rational b) => new Rational(a.Numerator * b.Numerator, a.Denomerator * b.Denomerator).Reduce();
    public static Rational operator /(Rational a, Rational b) => new Rational(a.Numerator * b.Denomerator, a.Denomerator * b.Numerator).Reduce();
    public static Rational operator +(Rational r1, int n2) => r1 + new Rational(n2, BigInteger.One, true);
    public static Rational operator -(Rational r) => new(-r.Numerator, r.Denomerator, r.reduced);
    public static Rational operator +(int n2, Rational r1) => r1 + n2;

    public static implicit operator Rational(int r) => new(r, BigInteger.One, true);
    public static implicit operator Rational(long r) => new(r, BigInteger.One, true);
    public static implicit operator Rational(string s) => Parse(s);
    ...
    {
        if (obj is not Rational rational) throw new Exception();
        return (Numerator * rational.Denomerator).CompareTo(Denomerator * rational.Numerator);
    }

    public static bool operator ==(Rational r1, Rational r2) => r1.Equals(r2);
    public static bool operator !=(Rational r1, Rational r2) => !r1.Equals(r2);
    public static bool operator <=(Rational r1, Rational r2) => r1.CompareTo(r2) < 1;
    ...
    public static Rational Max(params Rational[] values) => values.Max();
}
